...
import { HttpClient } from "choreapp-util/http";
...
const server = new ApolloServer({
  gateway: new ApolloGateway({
    serviceList: [
      ...
      { name: "chores", url: process.env.CHORES_FEDERATION_URL },
      { name: "notifications", url: process.env.NOTIFICATIONS_FEDERATION_URL }
    ],
    buildService({ url }) {
